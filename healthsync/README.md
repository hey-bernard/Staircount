...
3. 설정에서 **HTTP Push** (또는 Data Streaming) 기능을 켜고:
   ...
   - Batch 주기(배치 간격): **앱 기본값(1초)을 그대로 두셔도 됩니다** — Worker가 자체적으로 쓰기 빈도를 제한하기 때문에 하루 종일 켜놔도 안전합니다 (아래 "무료 플랜 한도" 참고)
4. 기록 시작
5. Staircount 앱의 "실시간 센서 스트림" 카드에서 **지금 확인** 또는 **5초마다 자동 갱신**으로 들어오는 원본 값을 확인

## 무료 플랜 한도 — 하루 종일 켜놔도 안전한 이유

Cloudflare Workers **KV의 무료 플랜은 하루 쓰기(write) 1,000건**이 한도예요 (읽기는 10만 건이라 넉넉합니다). Sensor Logger 기본 간격(1초)대로 하루 종일 스트리밍하면 86,400번 푸시가 들어오는데, 푸시마다 그대로 KV에 썼다면 **16분 만에 한도를 초과**했을 거예요.

그래서 `worker.js`가 **모든 푸시를 받아 응답은 하지만, 실제로 KV에 저장(쓰기)하는 건 최소 100초 간격**으로 스스로 제한합니다(`SENSOR_KV_WRITE_INTERVAL_MS`). 이렇게 하면:
- 하루 최대 864건만 KV에 씀 (1,000건 한도 안에서 여유 있게)
- 1초마다 아무리 많이 눌러도(또는 앱이 계속 보내도) 한도 초과 걱정 없이 **하루 종일 켜놔도 안전**
- 대신 화면에 보이는 값은 최대 100초 정도 지연될 수 있어요 — "완전 실시간"이 아니라 "최근 ~1분 40초 이내" 값이라고 보시면 됩니다
- 여러 센서를 나눠서 보내는 푸시도(예: 이번엔 Pedometer만, 다음엔 Barometer만) 서로 덮어쓰지 않고 **합쳐서(merge)** 저장하도록 함께 고쳤습니다
